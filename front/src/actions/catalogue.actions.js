...
export const createCatalogue = (catalogueData) => (dispatch) => {
  ...
  fetch('http://localhost:8089/catalogue/create', {
    method: 'POST',
...
export const getAllCatalogues = () => (dispatch) => {
  ...
  fetch('http://localhost:8089/catalogue/catalogues')
    .then((response) => response.json())
    .then((data) => dispatch({
...
export const getCatalogueById = (id) => async (dispatch) => {
    ...
    try {
      const response = await fetch(`http://localhost:8089/catalogue/catalogues/${id}`);
...
export const updateCatalogue = (id, catalogueData) => (dispatch) => {
  ...
  fetch(`http://localhost:8089/catalogue/update/${id}`, {
      method: 'PUT',
      body: formData,
  })
  .then(response => {
...
export const deleteCatalogue = (id) => (dispatch) => {
    ...
    fetch(`http://localhost:8089/catalogue/delete/${id}`, {
        method: 'DELETE',
    })
    .then(response => {
        if (!response.ok) {
...
export const countCatalogues = () => (dispatch) => {
  ...
  fetch('http://localhost:8089/catalogue/count')
    .then((response) => response.json())
    .then((data) => dispatch({
