...
export const addToCarttwo = (cartId, productId, quantity = 1) => async (dispatch) => { // Assigner 1 comme valeur par défaut pour quantity
    ...
    // Validation de productId pour s'assurer qu'il s'agit d'une chaîne de 24 caractères hexadécimaux
...
export const countCarts = () => async (dispatch) => {
    ...
    try {
        const response = await fetch('http://localhost:8089/carttwo/count');
        const data = await response.json();
        if (response.ok) {
            dispatch({ type: carttwoConstants.COUNT_CARTS_SUCCESS, payload: data.count });
        } else {
            throw new Error(data.message || "Failed to count carts");
        }
    } catch (error) {
        dispatch({ type: carttwoConstants.COUNT_CARTS_FAILURE, payload: error.message });
    }
};
