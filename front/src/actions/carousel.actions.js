...
export const createCarousel = (carouselData) => (dispatch) => {
  ...
  fetch('http://localhost:8089/carousel/create', {
    method: 'POST',
    body: formData,
  })
...
export const getAllCarousels = () => (dispatch) => {
  ...
  fetch('http://localhost:8089/carousel/carousels')
    .then((response) => response.json())
    .then((data) =>
      dispatch({
...
export const getCarouselById = (id) => async (dispatch) => {
  ...
  try {
    const response = await fetch(`http://localhost:8089/carousel/carousels/${id}`);
...
export const updateCarousel = (id, carouselData) => (dispatch) => {
  ...
  fetch(`http://localhost:8089/carousel/update/${id}`, {
    method: 'PUT',
    body: formData,
  })
    .then((response) => {
...
export const deleteCarousel = (id) => (dispatch) => {
  ...
  fetch(`http://localhost:8089/carousel/delete/${id}`, {
    method: 'DELETE',
  })
    .then((response) => {
      if (!response.ok) {
...
export const countCarousels = () => (dispatch) => {
  ...
  fetch('http://localhost:8089/carousel/count')
    .then((response) => response.json())
    .then((data) =>
      dispatch({
