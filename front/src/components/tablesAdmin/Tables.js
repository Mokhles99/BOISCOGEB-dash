...
const productSoustypes = [
'Vida','Saters','Moelven','Rundvirke','Vogue_bois','Calvi','Delta_Bois','Sangiorgi_Italie','Shared_Wood'
    ];

const productChoixs = [
             '1ere','2eme','3eme','6.5','Schalbord','7eme','Semi_avivé','Plots','Avivé_étuvé choix A','Plots_étuvé choix A','Plots_étuvé choix BC','Avivé_étuvé choix B',
...
const Tables = () => {
    ...
    const [productSoustype, setProductSoustype] = useState('');
    const [productChoix, setProductChoix] = useState('');
    ...
    const handleAddProduct = async () => {
        ...
        formData.append('choix', productChoix);
    ...
    const productColumns = [
        ...
        { field: 'choix', headerName: 'Choix', width: 150 },
                    ...
                    <Box component="form" noValidate autoComplete="off" sx={{ '& .MuiTextField-root': { mb: 2 } }}>
                     ...
                     <TextField
                        ...
                        <TextField
                            ...
                            select
                            value={productChoix}
                            onChange={(e) => setProductChoix(e.target.value)}
                        >
                            {productChoixs.map((category, index) => (
                                <MenuItem key={index} value={category}>
                                    {category}
                                </MenuItem>
                            ))}
    ...
    {item.product?.REF || 'Unknown'} &nbsp;/&nbsp;
    <span style={{ color: 'black', textDecoration: 'underline' }}>
        Product :
    </span> 
    {item.product?.name || 'Unknown'} &nbsp;(Qty: {item.quantity} m<sup>3</sup>)
</div>
